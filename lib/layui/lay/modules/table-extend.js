/**
  扩展了单选--singleSelect:true,// 1.扩展属性-单选--隐无为 2.showLoad:true 第一次加载和涮新表格出现加载loading
...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'hpTable', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
  ...
  Class.prototype.render = function(){
    ...
    if(options.showLoad){
    	if(options.height){
    		...
    		that.pullData(that.page,that.hpLoading());
    	}
  ...
  Class.prototype.renderForm = function(type){
    ...
    if(this.config.singleSelect){
       this.layHeader.find("th .layui-icon").hide();
        }
  }
  ...
  Class.prototype.loading = function(){
    ...
    //hp定制 请求loading  ==kd
  Class.prototype.hpLoading = function(){
    var that = this
    ,options = that.config;
    if(options.loading && options.url){
      return layer.msg('数据请求中', {
        icon: 16
        ,offset: [
          that.elem.offset().top + that.elem.height()/2 +120 - _WIN.scrollTop() + 'px'
          ,that.elem.offset().left + that.elem.width()/2 - 90 - _WIN.scrollLeft() + 'px'
        ]
        ,time: -1
        ,anim: -1
      ...
      if(that.config.singleSelect){
      	childs.each(function(i, item){
		        if(i==index){
		          ...
		          //设置勾选状态了
		          //不能直接 $(this),迂回
		          othis.next().addClass('layui-form-checked')
		        /* childs.each(function(i, item){
		         	console.log(i+":"+item.checked)
		         })*/
